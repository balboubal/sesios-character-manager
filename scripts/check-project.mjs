...
assert.equal(workbook.defaultState.schemaVersion, 8, "Other Skills persistence requires schema 8");
...
assert.match(bridge, /path === "otherSkills"/, "Dynamic Other Skills persistence hook is missing");
assert.match(bridge, /Other Skills/, "Other Skills panel is missing");
assert.match(bridge, /add-other-skill/, "Other Skills add control is missing");
assert.match(bridge, /remove-other-skill/, "Other Skills remove control is missing");
assert.equal(Array.isArray(workbook.defaultState.otherSkills), true, "Other Skills must have a clean default array");
...
assert.equal(legacySurvivalState.schemaVersion, 8, "Legacy state must migrate to schema 8");
...
assert.equal(craftingState.schemaVersion, 8);
